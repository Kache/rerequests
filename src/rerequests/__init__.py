# SPDX-FileCopyrightText: 2023-present Kevin C <Kache@users.noreply.github.com>
#
...
class Retry(urllib3.util.Retry):
    ...
    DEFAULT: Self = None  # type: ignore and assign later
    ...
    # TODO: configurable upper bound, don't obey when server tells you to sleep for 1000 years
    ...
    def _from_arg(cls, max_retries, **init_kwargs):
        return max_retries if isinstance(max_retries, cls) else cls(max_retries or 0, **init_kwargs)
...
class _ReSession(sessions.Session):
    ...
    def __init__(self, *, timeout=DEFAULT_TIMEOUT, max_retries: Retry | int | None = Retry.DEFAULT):
        super().__init__()
    ...
    def _for_convenience_api(cls, kwargs_for_request):
        max_retries = kwargs_for_request.pop('max_retries', None)
