...
import rerequests as requests
from rerequests import Retry
...
def test_retry_backoff_and_jitter():
    def iterate(retry: Retry):
        while True:
            yield retry
            try:
                retry = check_type(retry.increment(), Retry)  # b/c urllib3.util.Retry is incorrectly typed
            except urllib3.util.retry.MaxRetryError:
                break

    retries = list(iterate(Retry(9)))

    with patch('random.uniform') as mock_random_uniform:  # to remove jitter for tests
...
def test_init_defaults():
    assert requests.IDEMPOTENT_METHODS is urllib3.util.Retry.DEFAULT_ALLOWED_METHODS
    ...
    for method in ['GET', 'OPTIONS', 'HEAD', 'POST', 'PUT', 'PATCH', 'DELETE']:
        for err_status_code in range(400, 600):
            ...
            assert explicit_retry.is_retry(method, err_status_code) == is_transient
...
def test_resession_timeout():
    ...
    assert resolved_timeout(session={'timeout': 3}, request={'timeout': 0}) == 0
...
def test_resession_retry():
    ...
    assert retry_obj(requests._ReSession(max_retries=8)) == Retry(8, allowed_methods=requests.IDEMPOTENT_METHODS)

    assert retry_obj(requests._ReSession(max_retries=None)) is not urllib3.util.retry.Retry.DEFAULT
    assert retry_obj(requests._ReSession(max_retries=None)) == Retry(0, allowed_methods=requests.IDEMPOTENT_METHODS), "Should reproduce requests.HTTPAdapter default"
